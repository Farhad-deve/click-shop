      ...
      <Link to={`/products/${product.id}`} className="cursor-auto">
        <div className="border border-[#e5e7eb] dark:border-indigo-600 group hover:border-indigo-100 hover:dark:border-indigo-800 h-full duration-300 flex flex-col rounded-md overflow-hidden shadow-sm bg-white relative text-gray-700 hover:-translate-y-0.5">
          ...
          <div className="flex justify-center items-center p-2.5 border-b border-[#e5e7eb] dark:border-indigo-600">
            <img src={product.image} alt={product.name} className="max-h-42.5 min-h-42.5 sm:max-h-50 sm:min-h-50 w-full h-full object-contain" />
          </div>
          ...
          <div className="px-2.5 py-1.75 flex flex-col gap-2 justify-between flex-1 bg-slate-50 dark:bg-indigo-900 group-hover:bg-indigo-50 group-hover:dark:bg-indigo-800 duration-100">
            <h2 className="text-[14px] md:text-[16px] font-semibold dark:text-white">{product.name}</h2>          

            <div className="flex justify-start items-center text-[12px] font-medium gap-1">
              <span className="dark:text-white">{product.rate}</span>
            ...
            <div className="flex justify-between items-center">
              <div className="flex flex-col justify-between items-start">
                <div className="text-[12px] flex items-center gap-1">
                  <span className="line-through font-medium text-gray-500 dark:text-gray-400">$ {formatPrice(oldPrice)}</span>
